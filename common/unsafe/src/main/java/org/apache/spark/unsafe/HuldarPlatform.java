...
public class HuldarPlatform {
    ...
    private static final Field DBB_CLEARNER_FIELD;

    private static final Method CLEARNER_CREATE_METHOD;

    static {
        String cleanerClassName;
        if (majorVersion < 9) {
            cleanerClassName = "sum.misc.Cleaner";
        } else {
        ...
        try {
            ...
            DBB_CLEARNER_FIELD = cleanerField;

            if (DBB_CONSTRUCTOR != null && DBB_CLEARNER_FIELD != null) {
                ...
                CLEARNER_CREATE_METHOD = createMethod;
            } else {
                CLEARNER_CREATE_METHOD = null;
            }
    ...
    public static ByteBuffer allocateDirectBuffer(int size) {
        try {
            if (CLEARNER_CREATE_METHOD == null) {
                try {
            ...
            try {
                DBB_CLEARNER_FIELD.set(buffer,
                        CLEARNER_CREATE_METHOD.invoke(null, buffer, (Runnable)() -> freeMemory(memory)));
