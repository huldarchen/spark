...
object DataTypeUtils {
  ...
  /**
   * Compare two types, ingnoring nullability of ArrayType, MapType, StructType
   */
  def equalsIgnoreNullability(left: DataType, right: DataType): Boolean = {
    (left, right) match {
      ...
      case (StructType(leftFields), StructType(rigthFields)) =>
        leftFields.length == rigthFields.length &&
          leftFields.zip(rigthFields).forall { case (l, r) =>
  ...
  /**
   * Compare two types, ignoring nullability of ArrayType, MapType, StructType, and ignoring case
   * sensitivity of field names in StructType.
   */
  def equalsIgnoreCaseAndNullability(from: DataType, to: DataType): Boolean = {
    (from, to) match {
      case (ArrayType(fromElementType, _), ArrayType(toElementType, _)) =>
        equalsIgnoreCaseAndNullability(fromElementType, toElementType)
